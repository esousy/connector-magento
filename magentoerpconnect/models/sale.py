...
from odoo.addons.connector.exception import (NothingToDoJob,
                                                FailedJobError,
                                                IDMissingInBackend)
...
class SaleOrderImportMapper(ImportMapper):
    ...
    def customer_id(self, record):
        ...
        partner_id = binder.to_odoo(record['customer_id'], unwrap=True)
...
class SaleOrderImporter(MagentoImporter):
    ...
    def _must_skip(self):
        ...
        """
        if self.binder.to_odoo(self.magento_id):
    ...
    def _import_customer_group(self, group_id):
        ...
        if binder.to_odoo(group_id) is None:
    ...
    def _link_parent_orders(self, binding):
        ...
        for parent_id in all_parent_ids:
            parent_binding = self.binder.to_odoo(parent_id, browse=True)
    ...
    def _get_storeview(self, record):
        ...
        return storeview_binder.to_odoo(record['store_id'], browse=True)
    ...
    def _import_addresses(self):
        ...
        if is_guest_order:
            ...
            partner_binding = partner_binder.to_odoo(record['customer_id'],
                                                        browse=True)
...
class SaleOrderLineImportMapper(ImportMapper):
    ...
    def product_id(self, record):
        ...
        product_id = binder.to_odoo(record['product_id'], unwrap=True)
